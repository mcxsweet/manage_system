        ...
        <el-main v-if="ischoose">
            <el-card>
                <div class="clearfix">
                    <el-row>
                        <el-col :span="22">
                            <div @click="open(0)"><i class="el-icon-caret-right"></i> 课程教学总结</div>
                        </el-col>
                        <el-col :span="2">
            ...
            <el-card>
                <div class="clearfix">
                    <el-row>
                        <el-col :span="22">
                            <div @click="open(1)"><i class="el-icon-caret-right"></i> 今后改革设想</div>
                        </el-col>
                        <el-col :span="2">
                ...
                <div v-show="content[1].show">
                    <!-- <el-divider></el-divider> -->
                    <mavon-editor v-show="content[1].isedit" v-model="content[1].data" :subfield="false" @change="change"
    ...
    methods: {
        ...
        init() {
            api.get("/report/" + this.currentId + "/getImprovementActions", "", (resp) => {
                if (resp.data.data) {
                    this.content[0].data = resp.data.data.courseTeachingSummary;
                    this.content[1].data = resp.data.data.reformAssumption;
                }
            })
        },
        ...
        save(index) {
            switch (index) {
                case 0:
                    api.post("/report/InsertImprovementActions", { courseId: this.currentId, courseTeachingSummary: this.content[index].data }, (resp) => {
                        if (resp.data.flag) {
                            this.$message({
                                type: 'success',
                                message: '保存成功!'
                    ...
                    break;
                case 1:
                    api.post("/report/InsertImprovementActions", { courseId: this.currentId, reformAssumption: this.content[index].data }, (resp) => {
                        if (resp.data.flag) {
                            this.$message({
                                type: 'success',
                                message: '保存成功!'
