        ...
        <el-table-column label="操作" width="220">
          <template slot-scope="scope">
            <el-button
              type="primary"
              size="mini"
              @click="editStudent(scope.$index)"
              >编辑</el-button
            >
            <el-button
              type="success"
              size="mini"
              @click="saveStudent(scope.$index)"
              >保存</el-button
            >
            <el-button
              type="danger"
...
export default {
  ...
  methods: {
    ...
    getMessage() {
      api.get(
        "/courseInfo/currentUser/" + localStorage.getItem("UserId"),
        "",
        (resp) => {
    ...
    uploadFile() {
      ...
      axios
        .post(
          global.runTiemPath + "/student/" + this.currentId + "/studentInfo",
          formData,
          {
            headers: {
