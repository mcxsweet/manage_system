<template>
  <el-container>
    <el-main>
      <el-table :data="tableData1" border="true" style="width: 1284px" default-expand-all="true" > 
        ...
        <el-table-column prop="number2" label="项目百分比" width="170">
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.number2" :min="0" :max="100" style="width:140px" v-show="scope.row.ised1"></el-input-number>
            <p v-show="!scope.row.ised1">{{ scope.row.number2+'%' }}</p>
          </template>
        </el-table-column>
        <el-table-column label="考核子项目" width="280"> 
              <template slot-scope="scope">    
                  <el-select v-model="scope.row.childData" multiple placeholder="请选择" style="width:50%"  v-show="scope.row.ised2" ref="ad">
                ...
                <el-button icon="el-icon-plus" type="success" @click="addChild(scope.row,index)" v-show="scope.row.ised2" >确定</el-button> 
                <el-button icon="el-icon-plus" type="success" @click="setChild(scope.row,index)" v-show="!scope.row.ised2" >设置</el-button> 
    ...
    <div style="margin-top: 5%;" v-show="isdiv4">
        <p1>成绩评定法：期末总成绩=</p1>    
          <p1 v-if="scorelen>1">{{ this.score[0] }}</p1><p1 v-if="scorelen>1">{{ '('+this.score[scorelen/2]+'%)' }}</p1><p1 v-if="scorelen>3">+</p1>
          <p1 v-if="scorelen>3">{{ this.score[1] }}</p1><p1 v-if="scorelen>3">{{ '('+this.score[scorelen/2+1]+'%)'}}</p1><p1 v-if="scorelen>5">+</p1>
          <p1 v-if="scorelen>5">{{ this.score[2] }}</p1><p1 v-if="scorelen>5">{{ '('+this.score[scorelen/2+2]+'%)'}}</p1>
        <div>
          <p1 v-if="Usuallen>1">平时成绩=</p1>
          <p1 v-if="Usuallen>1">{{ this.Usual[0] }}</p1><p1 v-if="Usuallen>1">{{ '('+this.Usual[Usuallen/2]+'%)' }}</p1><p1 v-if="Usuallen>3">+</p1>
          <p1 v-if="Usuallen>3">{{ this.Usual[1] }}</p1><p1 v-if="Usuallen>3">{{ '('+this.Usual[Usuallen/2+1]+'%)' }}</p1><p1 v-if="Usuallen>5">+</p1>
          <p1 v-if="Usuallen>5">{{ this.Usual[2] }}</p1><p1 v-if="Usuallen>5">{{ '('+this.Usual[Usuallen/2+2]+'%)' }}</p1><p1 v-if="Usuallen>7">+</p1>
          <p1 v-if="Usuallen>7">{{ this.Usual[3] }}</p1><p1 v-if="Usuallen>7">{{ '('+this.Usual[Usuallen/2+3]+'%)' }}</p1>
         </div>
    ...
    <div v-show="!isdiv4">
      <el-form style="margin-top: 5%;">
        ...
        <el-form-item label="期末总成绩:" v-show="isdiv1">
          ...
          <el-form-item label="其他：" v-show="el=='其他'&&!isdiv1" v-for="(el,index) in score" :key="index" style="margin-left:50px ;">
            <el-input-number v-model="totalScore[0].elt[0].eltNum"  size="mini" :min="0" :max="100"></el-input-number>
          </el-form-item>
              <el-form-item label="平时成绩：" v-show="us=='平时成绩'&&!isdiv1" v-for="(us,index) in score" :key="index" style="margin-left:25px ;">
